...
public class RecordConverter implements RecordConvert{
    ...
    switch (eventType) {
      ...
      default:
        /*by crabo
    	  ...
    	  Record record = recordFactory.create(event.getOffset().format());
          Map<String, Field> fields = createHeader(event.getTable(), event.getEvent().getHeader(), event.getOffset());
          fields.put(TYPE_FIELD, create("QUERY"));
          Map<String, Field> data = new HashMap<>(1);
          data.put("Sql", create(evn.getSql()));
